...
- **Sign-in will be delegated**: no local passwords for new riders, because nothing in this codebase sends email, so there could be no verification and no reset (docs/adr/0007). `RequireUniqueEmail` also settles account linking — Identity cannot make a second account for an address that exists, so a second provider attaches to the same rider or fails.
- **Reads are not rider-scoped yet.** `UserId` is on every entity and the duplicate guard uses it, but all six read query handlers read the whole table and two of the queries don't even carry a user. Harmless with one rider, wrong with two — #156 fixes it, and it blocks everything else in #123 (docs/adr/0006).
...
- **Phase 3 is planned** (see the README roadmap): the interactive analysis view (#119), other sports (#120), weather enrichment (#121), and multi-user mode with Google/Microsoft login (#123). **#119 and #148 (the interactive analysis view, both directions) are built and merged**, as is **#121 (weather)**. #120 is grilled and `ready-for-agent`. **#123 is grilled and is now an umbrella**, cut into #156 (scope every read to a rider) → #157 (sign in with Google/Microsoft) → #158 (per-rider Polar linking, and a log that starts empty), in that dependency order. The last slice — what a regular rider may do to their own data — is deliberately not grilled yet. **The MapLibre swap (#122) was grilled and closed** — no capability wants it, and MapLibre would push the initial bundle (770 kB raw / 234 kB gzipped, error budget 1 MB) into needing a lazy-loading redesign first. So **#119 is built on Leaflet**; the old advice to swap engines before it no longer applies. Containerization and Blob Storage stay deferred too — they'd disturb the working zero-cost hosting for little visible gain.
...
- Test suite: **233 backend** (unit + endpoint tests in `RideLog.UnitTests`) and **286 frontend** (Vitest, 39 files).
