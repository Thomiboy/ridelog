...
- **Phase 1 is complete; Phase 2 is feature-complete but deliberately still open** — the owner is reviewing what to fine-tune, so new polish issues land in the Phase 2 milestone rather than Phase 3. The GitHub issue list is currently empty and everything through PR #116 is merged to `main`.
- **Phase 3 is planned** (see the README roadmap): the interactive analysis view (linked graph + map), weather enrichment, other sports, the MapLibre swap, and multi-user mode with Google/Microsoft login. Containerization and Blob Storage are deferred — they'd disturb the working zero-cost hosting for little visible gain.
