...
- **Phase 3 is planned** (see the README roadmap): the interactive analysis view (#119), weather enrichment (#120), other sports (#121), the MapLibre swap (#122), and multi-user mode with Google/Microsoft login (#123). All five are open and still need grilling — take #122 before #119 so the analysis view isn't built on Leaflet and then ported, and leave #123 for last. Containerization and Blob Storage are deferred — they'd disturb the working zero-cost hosting for little visible gain.
- Phase 2 delivered: statistics records and charts, the calendar view, ride comparison, rest markers, dark mode, Hungarian translation, locale-sensitive number/date/duration formatting (#104), two-year comparison on the Trends charts (#110), the Rides coverage backdrop (#113), the four-channel ride graph with speed (#114), the switcher sizing and mobile menu fix (#124), the map refit fix (#125), the streak and monthly records (#126), the same-month-last-year dashboard tiles (#129), the top-speed and biggest-climb records (#131), the monthly time chart (#132), and moving time as the canonical ride duration (#133).
- **No outstanding manual actions.** The admin **"Reprocess all"** has been run in production twice: once for the per-point **speed** added in #114, and again after #133 so existing GPX/TCX rides carry **moving time** as their duration (new syncs bring both in already). Every ride lost to a failed sync — including the Polar exercise `498528704` from 2026-07-23 — has been imported by hand via Polar Flow export → admin Import.
- Test suite: **184 backend** (unit + endpoint tests in `RideLog.UnitTests`) and **264 frontend** (Vitest, 36 files).
